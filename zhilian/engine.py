...
def check(claim, facts):
    ...
    out = {'claim_id': claim['id'], 'status': 'unverifiable', 'expected': None, 'reason': '', 'evidence': []}
    try:
        ...
        if k == 'quote':
            ...
            if len(refs) != len(s['values']):
                raise ValueError('图表关联数量与原始类别不一致')
            ...
            if any(f['metric'] != refs[0]['metric'] or f['scope'] != refs[0]['scope'] or f['subject'] != refs[0]['subject'] for f in refs):
                raise ValueError('图表系列的指标、主体或口径不一致')
            if [f['period'] for f in refs] != s['categories']:
                raise ValueError('图表类别与来源期间不匹配')
        ...
        out.update(status='consistent' if ok else 'inconsistent', expected=old if ok else expected, reason=calculation)
