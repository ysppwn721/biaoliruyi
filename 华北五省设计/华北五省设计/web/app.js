...
const imageKinds = ['png','jpg','gif','webp','bmp','tiff'];
...
function renderUploadFiles() {
  ...
  list.innerHTML = pendingUploads.map((file, index) => {
    ...
    const isImage = imageKinds.includes(ext);
    const type = ext === 'xlsx' ? 'Excel' : ext === 'docx' ? 'Word' : isImage ? '图片' : 'PPT';
    const icon = ext === 'xlsx' ? 'X' : ext === 'docx' ? 'W' : isImage ? '图' : 'P';
    const iconClass = ext === 'docx' ? 'docx' : ext === 'pptx' ? 'pptx' : isImage ? 'image' : '';
    return `<div class="upload-file-row"><span class="file-icon ${iconClass}">${icon}</span><div><strong>${esc(file.name)}</strong><small>${type} · ${(file.size / 1024 / 1024).toFixed(2)} MB</small></div><button type="button" class="icon-button upload-remove" data-upload-remove="${index}" aria-label="移除 ${esc(file.name)}">×</button></div>`;
  }).join('');
...
function downloadLinks() {
  return workspace.documents.map(d=>`<a class="button secondary small" href="${esc(d.download_url)}" download>↓ ${d.kind==='xlsx'?'Excel':d.kind==='docx'?'Word':d.kind==='pptx'?'PPT':'图片'} · ${esc(d.name)}</a>`).join('');
}
...
function deliveryModal() {
  ...
  if(canWriteBack())$('modalContent').insertAdjacentHTML('beforeend','<p class="hint">直接修改将覆盖本次导入时授权的原 Excel / Word / PPT。句柄仅在当前页面有效，刷新后需重新用支持写回的入口导入。</p>');
}
...
async function refreshProjects() {
  ...
  $('projects').innerHTML=projects.map(p=>`<div class="project-row"><button class="project-link ${workspace?.id===p.id?'active':''}" data-project="${p.id}">${esc(p.name)}</button><button class="icon-button project-delete" data-delete-project="${p.id}" data-delete-name="${esc(p.name)}" title="删除项目" aria-label="删除 ${esc(p.name)}">×</button></div>`).join('');
  return projects;
}
function deleteProjectModal(id, name){
  modal('删除项目',`<p class="modal-intro">确定删除项目「${esc(name)}」吗？该项目的全部数据、历史版本和导出文件都会被永久删除，且无法恢复。</p><div class="modal-actions"><button class="button secondary" id="cancelDelete">取消</button><button class="button primary" id="confirmDelete">确认删除</button></div>`);
  $('cancelDelete').onclick=closeModal;
  $('confirmDelete').onclick=async()=>{
    const result=await busy('正在删除项目',()=>api(`/api/projects/${id}`,{method:'DELETE'}));
    if(!result)return;
    closeModal();
    if(workspace?.id===id){workspace=null;localStorage.removeItem('zhilian-project');render();}
...
function render() {
  ...
  $('redoButton').disabled=!w.can_redo;
...
function uploadModal(autoWritable=false){
  ...
  modal('导入文件，建立项目',`<p class="modal-intro">支持的浏览器会自动打开可写文件选择器。选择后，修复完成时可直接覆盖本机原文件，无需额外勾选；也可继续使用普通上传。</p><p class="hint">一份 Excel 事实表，加一份或多份 Word / PPT / 图片。可以分多次选择，后续选择会加入列表，不会替换之前的文件。</p><form id="uploadForm"><label class="field-label" for="uploadName">项目名称</label><input id="uploadName" class="text-input" name="name" value="我的数据分析项目" maxlength="100" required><label class="dropzone"><h3>选择并加入文件</h3><p>可重复打开文件选择器；最多10份，合计不超过30MB</p><input id="uploadFiles" type="file" accept=".xlsx,.docx,.pptx,.png,.jpg,.gif,.webp,.bmp,.tiff" multiple></label>${window.showOpenFilePicker?'<button type="button" class="button secondary full" id="pickWritableFiles">选择文件（支持修改后写回原文件）</button><p class="hint">导入后可显式点击“直接修改本地文件”覆盖所选原文件，浏览器可能请求写入权限。刷新页面后需重新选择并导入；同名文件以最后一次选择为准。</p>':'<p class="hint">当前浏览器不支持原文件写回，请使用浏览器下载。</p>'}<div id="uploadFileList" class="upload-file-list"></div><div id="uploadFileStatus" class="upload-file-status">尚未加入文件</div><a class="link-button" href="/api/template">↓ 下载 Excel 事实表模板</a><p class="hint">需要一份 .xlsx 和至少一份 .docx / .pptx / 图片。Excel 首行包含：事实ID、主体、指标、期间、数值、单位、统计口径。</p><div class="modal-actions"><button type="button" class="button secondary" id="modalDemo">先体验演示</button><button class="button primary" id="uploadSubmit" type="submit" disabled>导入并识别</button></div></form>`);
  $('modalDemo').onclick=demo;
  const formElement=$('uploadForm'), handles=pendingFileHandles;
  const addFile=(file,handle)=>{
    const ext=file.name.split('.').pop().toLowerCase();
    if(!['xlsx','docx','pptx',...imageKinds].includes(ext))return;
  ...
  $('uploadForm').onsubmit=async e=>{
    ...
    if (pendingUploads.length < 2 || !extensions.includes('xlsx') || !extensions.some(ext => ['docx','pptx',...imageKinds].includes(ext))) { toast('请加入一份 Excel 和至少一份 Word / PPT / 图片', true); return; }
...
document.addEventListener('click',e=>{
  ...
  const del=e.target.closest('[data-delete-project]');if(del){deleteProjectModal(del.dataset.deleteProject,del.dataset.deleteName);return;}
...
$('redoButton').onclick=()=>{modal('重做被撤销的变更',`<p class="modal-intro">恢复刚刚被撤销的文件变更。操作记录会保留。</p><div class="modal-actions"><button class="button primary" id="applyRedo">确认重做</button></div>`);$('applyRedo').onclick=async()=>{if(await post('redo',{},'恢复被撤销的文件版本')){closeModal();toast('已恢复被撤销的文件变更');}};};
