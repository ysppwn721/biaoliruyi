...
def _tool(store, ws, name, payload=None, *, approved=False):
    ...
    model = llm.config()['model'] if name == 'suggest_links_llm' else None
...
def _diagnose(store, ws):
    started = perf_counter()
    diagnostics = []
    # 逐条论断调用 _tool 会为每条写盘两次，500 论断项目会产生约 2GB 写放大。
    # 这里在诊断循环内累积 trace/audit，按批一次性落盘，丢失的只是「逐条崩溃可恢复」的粒度。
    FLUSH_EVERY = 50
    pending_trace, pending_audit = [], []

    def flush():
        if not pending_trace:
            return
        ws['agent']['trace'].extend(pending_trace)
        ws['audit'].extend(pending_audit)
        pending_trace.clear()
        pending_audit.clear()
        _write(store, ws)
    ...
    for c in ws['claims']:
        ...
        entry = {'node': 'check_claim', 'at': now(), 'summary': '工具执行完成',
                 'model': None, 'duration_ms': round((perf_counter() - t0) * 1000)}
        pending_trace.append(entry)
        pending_audit.append({'time': entry['at'], 'event': 'Agent 执行', 'detail': 'check_claim：工具执行完成'})
        diagnostics.append(dict(_item(ws, c, checked), status=checked['status'], confirmed=c['confirmed']))
        if len(pending_trace) >= FLUSH_EVERY:
            flush()
    flush()
