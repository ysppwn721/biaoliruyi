...
class Store:
    ...
    def _prune_versions(self, ws):
        """清理过旧的版本目录，保留最近 N 代 + 初始 v0，避免磁盘无限增长。"""
        try:
            keep = int(os.getenv('ZHILIAN_KEEP_VERSIONS', '10'))
        except ValueError:
            keep = 10
        if keep <= 0:
            return
        root = self.folder(ws['id'])
        keep_gens = {ws['generation'], 'v0'}
        for entry in ws.get('history', [])[-keep:]:
            gen = entry.get('generation') if isinstance(entry, dict) else None
            if gen:
                keep_gens.add(gen)
        for child in root.iterdir():
            if child.is_dir() and child.name.startswith('v') and child.name not in keep_gens:
    ...
    def public(self, ws):
        ...
        result['can_redo'] = (self.folder(ws['id']) / 'redo-state.json').is_file()
    ...
    def undo(self, wid, revision):
        with self.lock:
            ...
            # 保留当前状态作为重做点（磁盘文件，避免 state.json 膨胀）。
            redo = copy.deepcopy(ws)
            redo['_redo_after_revision'] = ws['revision'] + 1
            redo_file = self.folder(wid) / 'redo-state.json'
            redo_file.write_text(json.dumps(redo, ensure_ascii=False, allow_nan=False), encoding='utf-8')
    ...
    def redo(self, wid, revision):
        with self.lock:
            ws = self.read(wid)
            self.verify(ws, revision)
            redo_file = self.folder(wid) / 'redo-state.json'
            if not redo_file.exists():
                raise ValueError('没有可以重做的文件变更')
            redo = json.loads(redo_file.read_text(encoding='utf-8'))
            # 撤销后若又发生了其他操作，重做点即失效，避免覆盖新状态。
            if redo.get('_redo_after_revision') != ws['revision']:
                redo_file.unlink(missing_ok=True)
                raise ValueError('重做点已因后续操作失效，请勿重复重做')
            redo.pop('_redo_after_revision', None)
            redo_file.unlink(missing_ok=True)
            redo['revision'] = ws['revision'] + 1
            redo['audit'] = ws['audit'] + [{'time': now(), 'event': '重做', 'detail': '恢复被撤销的文件变更'}]
            self.write(redo)
            return self.public(redo)
    ...
    def archive(self, wid):
        with self.lock:
            ...
            # 清理历史导出包，只保留最新一份，避免磁盘无限增长。
            for old_zip in root.glob('export-*.zip'):
                if old_zip.name != name:
                    try:
                        old_zip.unlink()
                    except OSError:
                        pass
